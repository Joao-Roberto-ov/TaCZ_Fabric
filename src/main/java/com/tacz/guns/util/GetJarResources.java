...
public final class GetJarResources {
    ...
    private static final Path BACKUP_PATH = Paths.get("config", GunModFabric.MOD_ID, "backup");
    ...
    public static void copyModFile(String srcPath, Path root, String path) {
        URL url = GunModFabric.class.getResource(srcPath);
        try {
            if (url != null) {
    ...
    public static void copyModDirectory(String srcPath, Path root, String path) {
        copyModDirectory(GunModFabric.class, srcPath, root, path);
    }

    @Nullable
    public static InputStream readModFile(String filePath) {
        URL url = GunModFabric.class.getResource(filePath);
        try {
            if (url != null) {
        ...
        try (ZipOutputStream zs = new ZipOutputStream(new FileOutputStream(tempFile));
            ...
            fileWalks.filter(Files::isRegularFile).forEach(path -> {
                ...
                try {
                    ...
                    GunModFabric.LOGGER.info("Error in zip file: {}", e.getMessage());
                }
            });
        }
        ...
        try {
            ...
            for (File file : delFiles) {
                if (count >= MAX_BACKUP_COUNT) {
                    // 超过十个的进行删除
                    GunModFabric.LOGGER.info("Deleting old backup gun pack {}", file.getName());
            ...
            GunModFabric.LOGGER.error("Error while checking old backup gun pack : {}", exception.getMessage());
        }
        return allMd5Hex;
    }
